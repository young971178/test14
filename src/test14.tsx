...
export interface UserBadgeRecord {
  user: UserName;
  badges: Record<ModuleId, BadgeLevel>;
}
...
export const USERS: { name: UserName; role: string; dept: string; avatarBg: string }[] = [
  { name: '박지성', role: '수석 연구원', dept: '디지털 혁신팀', avatarBg: 'bg-emerald-600' },
  { name: '이영표', role: '책임 엔지니어', dept: '스마트 팩토리팀', avatarBg: 'bg-blue-600' },
  { name: '류현진', role: '데이터 수석', dept: 'AI 데이터랩', avatarBg: 'bg-indigo-600' },
  { name: '손흥민', role: '선임 연구원', dept: '공정 데이터 분석팀', avatarBg: 'bg-amber-600' },
  { name: '박찬호', role: '마스터 마인드', dept: 'GDX 아카데미', avatarBg: 'bg-rose-600' },
];
...
const MODULE_ICONS: Record<ModuleId, React.ReactNode> = {
  기술통계: <BarChart3 className="w-6 h-6 text-blue-600" />,
  추론통계: <LineChart className="w-6 h-6 text-indigo-600" />,
  실험계획법: <FlaskConical className="w-6 h-6 text-emerald-600" />,
  통계적공정관리: <Activity className="w-6 h-6 text-violet-600" />,
  구조적문제해결방법론: <Workflow className="w-6 h-6 text-amber-600" />,
  미니탭리터러시: <Binary className="w-6 h-6 text-cyan-600" />,
  파이썬리터러시: <Code2 className="w-6 h-6 text-yellow-600" />,
  바이브코딩: <Sparkles className="w-6 h-6 text-pink-600" />,
  '머신러닝 이론': <Brain className="w-6 h-6 text-purple-600" />,
  탐색적데이터분석: <SearchCode className="w-6 h-6 text-teal-600" />,
  '머신러닝 모델링': <Cpu className="w-6 h-6 text-rose-600" />,
  'AI Automation': <Bot className="w-6 h-6 text-sky-600" />,
  시각화: <PieChart className="w-6 h-6 text-orange-600" />,
};
...
export const KolonBadge: React.FC<KolonBadgeProps> = ({
  ...
  const dimensions = {
    sm: { box: 'w-7 h-7', svg: 30, text: 'text-[9px]' },
    md: { box: 'w-10 h-10', svg: 40, text: 'text-[10px]' },
    lg: { box: 'w-14 h-14', svg: 56, text: 'text-xs' },
  }[size];
    ...
    <div className="inline-flex flex-col items-center justify-center gap-0.5">
      <div
        className={`relative flex items-center justify-center transition-all duration-200 ${
          isObtained ? 'hover:scale-110 cursor-pointer drop-shadow-sm' : 'opacity-70'
        }`}
      ...
      {showLabel && (
        <span
          className={`font-semibold tracking-tighter ${dimensions.text} ${
            isObtained ? 'text-slate-700 dark:text-slate-200' : 'text-slate-400 dark:text-slate-500'
          }`}
        >
          {type}
        </span>
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-slate-900 text-white border-b border-slate-800 shadow-md px-4 md:px-6 py-3 shrink-0">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-600 to-rose-700 flex items-center justify-center shadow-md border border-red-400/30 p-1.5">
            <svg
              width="28"
              height="28"
              viewBox="0 0 100 100"
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="flex items-center gap-2 bg-slate-800/90 border border-slate-700 rounded-xl px-3 py-1.5 shadow-xs">
            <div
              className={`w-7 h-7 rounded-lg ${activeUserData.avatarBg} text-white flex items-center justify-center font-bold text-xs shadow-2xs`}
            >
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold px-3 py-2 rounded-xl transition-all shadow-xs cursor-pointer"
...
export const BadgeMatrix: React.FC<BadgeMatrixProps> = ({ currentUser }) => {
  return (
    <div className="bg-white/90 backdrop-blur-md rounded-2xl border border-slate-200/80 shadow-xs p-3 md:p-4 flex flex-col justify-between h-full">
      <div className="flex items-center justify-between border-b border-slate-100 pb-2 mb-2">
        <div className="flex items-center gap-2">
          <div className="p-1.5 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-lg text-white shadow-xs">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500">
              노랑 → 녹색 → 검정 → 실버크라운 → 골드크라운 순차 누적 획득
            </p>
          </div>
        </div>

        <div className="hidden lg:flex items-center gap-2 bg-slate-50 px-2.5 py-1 rounded-lg border border-slate-100">
          <span className="text-[11px] font-medium text-slate-500 mr-1">배지 단계:</span>
          {BADGE_TYPES.map((type) => (
            <div key={type} className="flex items-center gap-1">
              <KolonBadge type={type} isObtained={true} size="sm" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 overflow-y-auto max-h-[calc(100%-3rem)] pr-0.5">
        {MODULE_ORDER.map((moduleId) => {
          ...
          return (
            <div
              key={moduleId}
              className="bg-slate-50/70 hover:bg-slate-100/80 transition-colors duration-150 rounded-xl p-1.5 md:p-2 border border-slate-200/60 flex flex-col justify-between gap-1"
            >
              <div className="flex items-center justify-between gap-1">
                <span className="font-bold text-xs text-slate-800 tracking-tight truncate" title={moduleId}>
                  {moduleId}
                </span>
                {userLevel > 0 ? (
                  <span className="inline-flex items-center gap-0.5 text-[9px] md:text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200/60 shrink-0">
                  ...
                  <span className="text-[9px] md:text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-slate-200/70 text-slate-500 shrink-0">
                    미획득
                  </span>
                )}
              </div>

              <div className="flex items-center justify-between px-1 py-1 bg-white rounded-lg border border-slate-100 shadow-2xs">
...
export const ModuleGrid: React.FC<ModuleGridProps> = ({
  ...
  return (
    <div className="bg-white/90 backdrop-blur-md rounded-2xl border border-slate-200/80 shadow-xs p-3 md:p-4 flex flex-col justify-between h-full">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2.5 overflow-y-auto max-h-[calc(100%-3rem)] pr-0.5">
        {MODULE_ORDER.map((id) => {
  ...
  return (
    <div className="min-h-screen h-screen bg-slate-100 flex flex-col text-slate-900 font-sans overflow-hidden select-none">
      ...
      {activeModule ? (
        ...
        <main className="flex-1 flex flex-col gap-3 p-3 md:p-4 h-[calc(100vh-61px)] overflow-hidden">
          <section className="h-[48%] min-h-0 w-full">
            <BadgeMatrix currentUser={currentUser} />
          </section>
